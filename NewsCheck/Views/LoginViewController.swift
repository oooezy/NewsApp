...
//    var userModel = UserModel()
    ...
    func setupControl() {
        ...
        loginViewModel.isValid
            .map { $0 ? .mainColor : .clear }
            .bind(to: loginButton.rx.backgroundColor)
            .disposed(by: disposeBag)
        
//        loginButton.rx.tap.subscribe(
//            onNext: { [weak self] _ in
//                if self?.userEmail == self?.loginViewModel.emailObserver.value && self?.userPassword == self?.loginViewModel.passwordObserver.value { // 서버에 저장된 이메일과 입력한 비번이 같으면
//                    print("로그인 성공")
//                } else {
//                    print("로그인 실패")
//                }
